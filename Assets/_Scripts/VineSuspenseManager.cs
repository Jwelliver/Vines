...
public class VineSuspenseManager : MonoBehaviour
{
    ...
    VineRoot vineRoot;
    public static bool suspendOnStart = true; // flag that controls whether instances should automatically try to suspend themselves on start; Used externally to keep initially loaded vines unsuspended while still allowing instances to suspend themselves without being instructed externally.
    void Awake()
    {
    ...
    // ? 110123 - Commenting these out to test the VineActiveTrigger vine suspension method
...
/// ====  110123 WIP attempt to have a static VineSuspenseManager section that handles intermittently unsuspend and resuspend vines near to the player for small sections of time until all have achieved a minimum time spend unsuspended; Ended up running into some weird, unanticipated issues; Was debugging, but now replacing with a system that is managed by a VineActiveTrigger attached to the player;
// public class VineSuspenseManager : MonoBehaviour
// {
//     public float suspensionWaitSeconds = 5f; // how many seconds to wait between triggering suspend and executing it
//     public bool isVineSuspended;
...
//     public static void UnsuspendAllNotVisible()
//     {
//         foreach (VineSuspenseManager vineSuspenseManagerInstance in VineSuspenseManagerInstancesRef)
//         {
//             vineSuspenseManagerInstance.UnsuspendSegments();
//         }
//     }
