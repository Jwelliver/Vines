...
- To do the initial time fastforward, we could maybe use a static event in VineRoot, each root subscribes it's InitSuspend to the event; then, after the time forward is called and completed elsewhere, gamemanager, or levelgen can invoke the static event? After complete; each root can remove it's own method or manager does removeAll

  - optionally, to do them all "fast" instead of via random time intervals; Could maybe use a static queue in vineroot;
    ... each vineRoot that is to be suspended on start adds itself to the queue, then a static coroutine works through the queue and suspends a single vineRoot each frame, the WaitForNewFrame and repeat until the queue is empty
...
- Arrows should aim at player +/- some degree; This ensures levels with higher trees don't allow the player to climb out of range.
  ...
  - Arrows should be maintained in an object pool

- -! Normals: added 100623:

  - review; ensure import settings are correct (no mimaps; correct compression, etc.)
  - Assign to parent sprites; repack atlases;

  * figure out how/if to add specular map
  * Dont forget to reset existing light's normal interaction(lightshaft prefab and tempsun)

  - remove existing normals on adornments, vines, platform; not in use (scene too dark; May be better with spec map if you figure that out)
...
- To make Water look like it's passing by; Try to figure out how to apply the texture in the shader at world position ?
  ...
  - Add options screen // Added;
    ...
    - quality settings:
      ...
      - lighting quality (adjust light texture)
        ...
        - control whether lights interact with normals
        - figure out how we're handling normals being added in or not
...
- Transitions:
  - consider using a "slide to" from mainMenu to the settings or other menus.
    - i.e. you have the settings menu offscreen, then just slide over to it and back instead of it living in a separate scene
...
- Cache all refs (instead of extern calls e.g. GameObject.GetComponent, etc.)
- Check phone notes Vines 100623
- Using tween on UI is preferable to animator; Replace crossfade anim with tween.
- GameMain: Gui repaint is spiking ~368B of GC every frame
  - figure out which UIs are responsible; Disable until needed; there may be a script updating a Text even though the text is transparent.
- There is about ~2.7kb of GC alloc happening regularly; Track it down
  - note: try enabling profile ediotr and see if it's coming from there; Ultimately profile the build to finalize
- See vines section about vine root
- Cache List.Count; Also, switch to array where applicable
  - already did this for the vines;
  - Definitely review factories and everywhere else.
...
- Tested using suspension in VineRoot;

  - verify results and perf;
  - Add in a coroutine to create a delay before resuspending; see todo in module
    - once worked out, try using interpolation and recheck perf;
